...
def build_arg_parser() -> ArgumentParser:
    ...
    open_parser = subparsers.add_parser(
        "open", help="Open a workspace when the app starts"
    )
...
def list_connections(console: Console, *, start: Path | None = None) -> None:
    ...
    table = Table(
        title="Configured Connections", box=box.ROUNDED, header_style="bold cyan"
    )
    ...
    console.print(
        "  • Open a connection: [green]cockpit-cli open --connection <alias>[/green]"
    )
...
def list_connections_text(*, start: Path | None = None) -> str:
    """Helper for tests and simple CLI output."""
    service = ConnectionService(ConfigLoader(start=start))
    profiles = service.list_profiles()
    if not profiles:
    ...
    for profile in profiles:
        lines.append(f"- {profile.alias}: {profile.target_ref} ({profile.description or 'No description'})")
    return "\n".join(lines)
...
def list_datasources(console: Console, *, start: Path | None = None) -> None:
    ...
    try:
        ...
        if not profiles:
            ...
            console.print(
                "  • Run query:      [dim]Use the 'DB' tab inside the TUI[/dim]"
            )
            console.print()
    finally:
        container.shutdown()
        ...
        if getattr(args, "subcommand", None) == "admin":
            ...
            try:
                ...
                while True:
                    ...
                    if (
                        listen_url
                        and getattr(args, "open_browser", False)
                        and not opened_browser
                    ):
