...
from cockpit.ops.models.review import ActionItem, ReviewFinding
...
from cockpit.ops.models.watch import ComponentWatchConfig
...
class WebAdminService:
    ...
    def diagnostics(self) -> dict[str, object]:
        ...
        for incident in active_incidents[:5]:
            ...
            recent_attempts.extend(
                attempt.to_dict() for attempt in detail.recovery_attempts
            )
        ...
        return {
            ...
            "quarantined_components": [
                state.to_dict()
                for state in self._self_healing_service.list_quarantined()
            ],
            ...
            "suppression_rules": [
                rule.to_dict() for rule in self._suppression_service.list_rules()
            ],
            "oncall": {
                "people": [
                    person.to_dict() for person in self._oncall_service.list_people()
                ],
                ...
                "schedules": [
                    schedule.to_dict()
                    for schedule in self._oncall_service.list_schedules()
                ],
            ...
            "runbooks": [
                runbook.to_dict()
                for runbook in self._runbook_catalog_service.list_runbooks()
            ],
            "reviews": [
                review.to_dict()
                for review in self._postincident_service.list_reviews(limit=25)
            ],
            "watches": {
    ...
    def create_datasource(self, payload: dict[str, object]) -> DataSourceProfile:
        ...
        return self._datasource_service.create_profile(
            ...
            or payload.get("backend", "Datasource"),
    ...
    def save_notification_channel(
        ...
        existing = next(
            (
                item
                for item in self.list_notification_channels()
        ...
        channel = NotificationChannel(
            id=channel_id
            or NotificationPolicyService.new_channel(
            ...
            secret_refs={
                str(key): str(value)
                for key, value in self._json_mapping(
                    payload.get("secret_refs_json")
                ).items()
            },
    ...
    def save_notification_rule(self, payload: dict[str, object]) -> NotificationRule:
        ...
        rule = NotificationRule(
            id=rule_id
            or NotificationPolicyService.new_rule(
    ...
    def save_suppression_rule(
        ...
        existing = next(
            (
                item
                for item in self.list_suppression_rules()
                if item.id == suppression_id
            ),
            None,
        )
        now = utc_now()
        rule = NotificationSuppressionRule(
            id=suppression_id
            or self._suppression_service.new_rule(
    ...
    def list_watch_states(self) -> list[dict[str, object]]:
        return [
            state.to_dict() for state in self._component_watch_service.list_states()
        ]
    ...
    def save_datasource_watch(self, payload: dict[str, object]) -> ComponentWatchConfig:
        ...
        existing = (
            self._component_watch_service.get_config(watch_id) if watch_id else None
        )
        config = existing or ComponentWatchService.new_datasource_watch(
            profile_id=profile_id
        )
        ...
        config.probe_interval_seconds = int(
            payload.get("probe_interval_seconds", 30) or 30
        )
        config.stale_timeout_seconds = int(
            payload.get("stale_timeout_seconds", 90) or 90
        )
    ...
    def save_docker_watch(self, payload: dict[str, object]) -> ComponentWatchConfig:
        ...
        config = existing or ComponentWatchService.new_docker_watch(
    ...
    def validate_layout_document(self, payload: dict[str, object]) -> dict[str, object]:
        panel_types = {
            panel_type
            for panel_type, _panel_id, _display_name in self.available_panels()
        }
        panel_ids = {
            panel_id for _panel_type, panel_id, _display_name in self.available_panels()
        }
    ...
    def toggle_layout_tab(self, layout_id: str, tab_id: str) -> Layout:
        return self._layout_service.toggle_tab_orientation(
            layout_id=layout_id, tab_id=tab_id
        )

    def set_layout_ratio(self, layout_id: str, tab_id: str, ratio: float) -> Layout:
        return self._layout_service.set_tab_ratio(
            layout_id=layout_id, tab_id=tab_id, ratio=ratio
        )
    ...
    def remove_panel_from_layout(
        self, layout_id: str, tab_id: str, panel_id: str
    ) -> Layout:
    ...
    def list_incidents(
        ...
        *,
        ...
        statuses = (
            (IncidentStatus(status),) if isinstance(status, str) and status else None
        )
        severities = (
    ...
    def save_operator_person(self, payload: dict[str, object]) -> OperatorPerson:
        ...
        person = OperatorPerson(
            id=person_id
            or self._oncall_service.new_person(
    ...
    def save_operator_team(self, payload: dict[str, object]) -> OperatorTeam:
        ...
        team = OperatorTeam(
            id=team_id
            or self._oncall_service.new_team(
    ...
    def save_team_membership(self, payload: dict[str, object]) -> TeamMembership:
        ...
        membership = TeamMembership(
            id=membership_id
            or f"mem:{payload.get('team_id', '')}:{payload.get('person_id', '')}",
    ...
    def save_ownership_binding(self, payload: dict[str, object]) -> OwnershipBinding:
        ...
        existing = next(
            (
                item
                for item in self._oncall_service.list_ownership_bindings()
        ...
        binding = OwnershipBinding(
            ...
            team_id=str(payload.get("team_id", "")),
            enabled=str(payload.get("enabled", "1")) != "0",
            component_kind=self._optional_enum(
                payload.get("component_kind"), ComponentKind
            ),
            component_id=self._optional_str(payload.get("component_id")),
            subject_kind=self._optional_enum(
                payload.get("subject_kind"), OwnershipSubjectKind
            ),
            ...
            escalation_policy_id=self._optional_str(
                payload.get("escalation_policy_id")
            ),
            created_at=existing.created_at if existing is not None else now,
            updated_at=now,
        )
    ...
    def save_oncall_schedule(self, payload: dict[str, object]) -> OnCallSchedule:
        ...
        existing = next(
            (
                item
                for item in self._oncall_service.list_schedules()
        ...
        schedule = OnCallSchedule(
            id=schedule_id
            or f"sch:{payload.get('team_id', '')}:{payload.get('name', '')}",
            team_id=str(payload.get("team_id", "")),
            name=str(payload.get("name", "")),
            timezone=str(payload.get("timezone", "UTC")),
            enabled=str(payload.get("enabled", "1")) != "0",
            coverage_kind=ScheduleCoverageKind(
                str(payload.get("coverage_kind", "always"))
            ),
    ...
    def save_rotation(self, payload: dict[str, object]) -> RotationRule:
        ...
        existing = next(
            (
                item
                for item in self._oncall_service.list_rotations(schedule_id)
        ...
        rotation = RotationRule(
            ...
            interval_kind=RotationIntervalKind(
                str(payload.get("interval_kind", "days"))
            ),
    ...
    def save_override(self, payload: dict[str, object]) -> ScheduleOverride:
        ...
        existing = next(
            (
                item
                for item in self._oncall_service.list_overrides(schedule_id)
        ...
        override = ScheduleOverride(
            id=override_id
            or f"ovr:{schedule_id}:{payload.get('replacement_person_id', '')}:{now.timestamp()}",
            schedule_id=schedule_id,
    ...
    def save_escalation_policy(self, payload: dict[str, object]):
        ...
        policy = EscalationPolicy(
            id=policy_id
            or self._escalation_policy_service.new_policy(
            ...
            created_at=(
                existing_detail.policy.created_at
        ...
        for index, item in enumerate(steps_payload):
            ...
            steps.append(
                EscalationStep(
                    id=step_id
                    or self._escalation_policy_service.new_step(
                        step_index=index,
                        target_kind=EscalationTargetKind(
                            str(item.get("target_kind", "team"))
                        ),
    ...
    def acknowledge_engagement(self, engagement_id: str, *, actor: str = "web-admin"):
        return self._escalation_service.acknowledge_engagement(
            engagement_id, actor=actor
        )
