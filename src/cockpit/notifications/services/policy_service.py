...
from cockpit.core.enums import NotificationChannelKind, TargetRiskLevel
...
class NotificationPolicyService:
    ...
    def delete_channel(self, channel_id: str) -> None:
        if channel_id == self._DEFAULT_CHANNEL_ID:
            raise ValueError(
                "The default internal notification channel cannot be deleted."
            )
    ...
    def enabled_channels_for_ids(
        ...
        return [
            enabled[channel_id] for channel_id in channel_ids if channel_id in enabled
        ]
    ...
    def _matches_rule(candidate: NotificationCandidate, rule: NotificationRule) -> bool:
        ...
        if rule.risk_levels and candidate.risk_level not in rule.risk_levels:
            return False
        if (
            candidate.incident_status is not None
            and rule.incident_statuses
            and candidate.incident_status not in rule.incident_statuses
        ):
            return False
        return True
