...
from cockpit.ops.models.review import PostIncidentReview
...
class ResponseRunService:
    ...
    def get_response_detail(self, run_id: str) -> ResponseRunDetail | None:
        ...
        return ResponseRunDetail(
            ...
            compensations=tuple(
                self._compensation_repository.list_for_run(response_run.id)
            ),
    ...
    def start_run(
        ...
        *,
        ...
        existing = self._response_run_repository.find_active_for_incident(incident_id)
        if existing is not None:
            raise ValueError(
                f"Incident '{incident_id}' already has an active response run."
            )
        runbook = self._runbook_catalog_service.get_runbook(
            runbook_id, version=runbook_version
        )
        started_at = utc_now()
        response_run = ResponseRun(
            id=make_id("rrn"),
        ...
        self._step_run_repository.save(
            self._new_step_run(response_run, runbook.steps[0], step_index=0)
        )
    ...
    def execute_current_step(
        ...
        *,
        ...
        step_run = self._require_current_step_run(
            response_run.id, response_run.current_step_index
        )
        if response_run.status in {
            ResponseRunStatus.COMPLETED,
            ResponseRunStatus.ABORTED,
        }:
            raise ValueError("Closed response runs cannot execute more steps.")
        approved_request = self._approval_request_repository.get_latest_for_step(
            step_run.id
        )
        ...
        if step_definition.requires_confirmation and not confirmed:
            ...
            return response_run
        if step_definition.requires_elevated_mode and not (
            elevated_mode or response_run.elevated_mode
        ):
            self._set_waiting_operator(
                response_run,
                step_run,
        ...
        self._set_run_status(
            response_run,
            ResponseRunStatus.RUNNING,
            f"Executing step {step_definition.key}.",
        )
    ...
    def retry_current_step(
        ...
        *,
        ...
        if (
            step_run.attempt_count >= max_attempts
            and step_run.status is ResponseStepStatus.FAILED
        ):
        ...
        self._step_run_repository.save(step_run)
        self._set_run_status(
            response_run, ResponseRunStatus.READY, "Retrying current step."
        )
    ...
    def compensate_latest_step(
        ...
        *,
        ...
        target_step_run, target_definition = self._latest_compensatable_step(
            response_run, runbook
        )
        ...
        if compensation.approval_required:
            request = self._approval_request_repository.get_latest_for_step(
                target_step_run.id
            )
            if request is None:
                self._move_to_waiting_approval(
        ...
        if compensation.requires_confirmation and not confirmed:
            self._set_waiting_operator(
                response_run, target_step_run, "Compensation requires confirmation."
            )
            return response_run
        if compensation.requires_elevated_mode and not (
            elevated_mode or response_run.elevated_mode
        ):
            self._set_waiting_operator(
                response_run, target_step_run, "Compensation requires elevated mode."
            )
            return response_run
        ...
        self._set_run_status(
            response_run,
            ResponseRunStatus.COMPENSATING,
            f"Running compensation for {target_definition.key}.",
        )
        self._event_bus.publish(
            CompensationStatusChanged(
        ...
        if outcome.waiting_for_operator:
            ...
            self._compensation_repository.save(compensation_run)
            self._set_waiting_operator(
                response_run, target_step_run, outcome.result.summary
            )
            return response_run
        if outcome.blocked or not outcome.result.success:
            ...
            self._compensation_repository.save(compensation_run)
            self._set_run_status(
                response_run, ResponseRunStatus.BLOCKED, outcome.result.summary
            )
            self._event_bus.publish(
                CompensationStatusChanged(
                    compensation_run_id=compensation_run.id,
        ...
        self._set_run_status(
            response_run,
            ResponseRunStatus.ABORTED,
            "Response run compensated and aborted.",
        )
        self._event_bus.publish(
            CompensationStatusChanged(
                compensation_run_id=compensation_run.id,
    ...
    def decide_approval(
        ...
        *,
        ...
        if request.status is ApprovalRequestStatus.APPROVED:
            ...
            self._set_run_status(
                response_run, ResponseRunStatus.READY, "Approval request approved."
            )
        ...
        *,
        ...
        if outcome.result.success:
            ...
            if next_index >= len(runbook.steps):
                response_run.completed_at = utc_now()
                self._set_run_status(
                    response_run, ResponseRunStatus.COMPLETED, "Response run completed."
                )
                self._postincident_service.ensure_review(
                    incident_id=incident.id,
                    response_run_id=response_run.id,
            ...
            self._response_run_repository.save(response_run)
            if (
                self._step_run_repository.get_by_run_and_index(
                    response_run.id, next_index
                )
                is None
            ):
                self._step_run_repository.save(
                    self._new_step_run(
                        response_run, runbook.steps[next_index], step_index=next_index
                    )
                )
            self._set_run_status(
                response_run, ResponseRunStatus.READY, "Next response step is ready."
            )
            return response_run
        ...
        if step_definition.continue_on_failure:
            ...
            if next_index < len(runbook.steps):
                ...
                self._set_run_status(
                    response_run,
                    ResponseRunStatus.READY,
                    "Continuing after step failure.",
                )
                return response_run
        self._set_run_status(
            response_run, ResponseRunStatus.FAILED, outcome.result.summary
        )
        return response_run
    ...
    def _move_to_waiting_approval(
        ...
        *,
        ...
        self._set_run_status(
            response_run,
            ResponseRunStatus.WAITING_APPROVAL,
            f"Waiting for approval request {request.id}.",
        )
    ...
    def _latest_compensatable_step(
        ...
        step_runs = list(
            reversed(self._step_run_repository.list_for_run(response_run.id))
        )
        for step_run in step_runs:
            if step_run.status not in {
                ResponseStepStatus.SUCCEEDED,
            ...
            if step_definition.compensation is None:
                continue
            latest_compensation = self._compensation_repository.latest_for_step(
                step_run.id
            )
            if (
                latest_compensation is not None
                and latest_compensation.status is CompensationStatus.COMPLETED
            ):
                continue
    ...
    def _on_incident_status_changed(self, event: IncidentStatusChanged) -> None:
        if event.new_status not in {IncidentStatus.RESOLVED, IncidentStatus.CLOSED}:
            return
        response_run = self._response_run_repository.find_active_for_incident(
            event.incident_id
        )
        if response_run is None:
            return
