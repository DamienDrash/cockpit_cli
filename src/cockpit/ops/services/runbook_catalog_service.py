...
from cockpit.core.config import runbooks_dir
...
class RunbookCatalogService:
    ...
    def reload(self) -> list[RunbookDefinition]:
        ...
        definitions = [_with_loaded_at(runbook) for runbook in self._loader.discover()]
    ...
    def get_runbook(
        ...
        if runbook is None:
            raise LookupError(
                f"Runbook '{runbook_id}' version '{version}' was not found."
            )
        return runbook
