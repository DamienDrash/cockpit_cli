...
class ResponseExecutorService:
    ...
    def execute_step(
        ...
        *,
        ...
        if (
            ...
            and guard_decision.outcome is not GuardDecisionOutcome.ALLOW
        ):
            return ResponseExecutionOutcome(
                ...
                waiting_for_operator=guard_decision.outcome
                in {
    ...
    def _resolve_config(
        ...
        *,
        ...
        return cast(dict[str, object], _resolve_value(config, replacements))
...
def _resolve_value(value: object, replacements: dict[str, str]) -> object:
    ...
    if isinstance(value, dict):
        return {
            str(key): _resolve_value(item, replacements) for key, item in value.items()
        }
    return value
