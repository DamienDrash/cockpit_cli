...
from cockpit.ops.events.health import IncidentOpened, IncidentStatusChanged
...
class EscalationService:
    ...
    def __init__(
        ...
        *,
        ...
        self._event_bus.subscribe(
            NotificationDelivered, self._on_notification_delivered
        )
        self._event_bus.subscribe(
            NotificationDeliveryFailed, self._on_notification_delivery_failed
        )
    ...
    def get_engagement_detail(self, engagement_id: str) -> EngagementDetail | None:
        ...
        return EngagementDetail(
            ...
            incident=incident,
            timeline=tuple(
                self._timeline_repository.list_for_engagement(engagement_id)
            ),
            delivery_links=tuple(
                self._delivery_link_repository.list_for_engagement(engagement_id)
            ),
        )

    def acknowledge_engagement(
        self, engagement_id: str, *, actor: str
    ) -> IncidentEngagement:
        engagement = self._require_engagement(engagement_id)
        if engagement.status in {
            EngagementStatus.RESOLVED,
            EngagementStatus.CLOSED,
            EngagementStatus.EXHAUSTED,
        }:
    ...
    def repage_engagement(
        self, engagement_id: str, *, actor: str | None = None
    ) -> IncidentEngagement:
        engagement = self._require_engagement(engagement_id)
        if engagement.status in {
            EngagementStatus.RESOLVED,
    ...
    def diagnostics(self) -> dict[str, object]:
        ...
        blocked = [
            item.to_dict() for item in active if item.status is EngagementStatus.BLOCKED
        ]
        exhausted = [
            item.to_dict()
    ...
    def _on_incident_opened(self, event: IncidentOpened) -> None:
        ...
        if incident is None:
            return
        if self._engagement_repository.find_active_for_incident(incident.id) is not None:
            return
    ...
    def _on_incident_status_changed(self, event: IncidentStatusChanged) -> None:
        if event.new_status not in {IncidentStatus.RESOLVED, IncidentStatus.CLOSED}:
            return
        engagement = self._engagement_repository.find_active_for_incident(
            event.incident_id
        )
        if engagement is None:
            return
    ...
    def _on_notification_delivered(self, event: NotificationDelivered) -> None:
        for link in self._delivery_link_repository.list_for_notification(
    ...
    def _on_notification_delivery_failed(
        self, event: NotificationDeliveryFailed
    ) -> None:
        for link in self._delivery_link_repository.list_for_notification(
            event.notification_id
        ):
    ...
    def _create_engagement_for_incident(
        ...
        policy = self._escalation_policy_service.get_policy(
            ownership.escalation_policy_id or ""
        )
        ...
        self._timeline_repository.add_entry(
            ...
            payload={
                ...
                "recipient": recipient.explanation,
            },
        )
        self._event_bus.publish(
            IncidentEngagementCreated(
    ...
    def _run_due_action(
        self, engagement: IncidentEngagement, *, effective_now: datetime
    ) -> None:
        incident = self._require_incident(engagement.incident_id)
        ...
        if engagement.ack_deadline_at and effective_now >= engagement.ack_deadline_at:
            ...
            engagement.resolved_person_id = recipient.person_id
            engagement.repeat_page_count = 0
            engagement.ack_deadline_at = (
                self._escalation_policy_service.ack_deadline_for(
                    policy=policy,
                    step=next_step,
                    effective_at=effective_now,
                )
            ...
            if recipient.outcome is ResolutionOutcome.RESOLVED:
                engagement.status = EngagementStatus.ACTIVE
                engagement.next_action_at = (
                    self._escalation_policy_service.next_repeat_at(
                        policy=policy,
                        step=next_step,
                        effective_at=effective_now,
                    )
        ...
        max_repeats = self._escalation_policy_service.max_repeat_pages(
    ...
    def _page_engagement(
        ...
        *,
        ...
        event_class = self._notification_event_class_for(
            purpose, engagement.current_step_index
        )
        ...
        self._operations_diagnostics_service.record_operation(
            ...
            success=notification.status is not None
            and notification.status.value != "failed",
    ...
    def _require_policy(self, engagement: IncidentEngagement):
        ...
        if policy is None:
            raise LookupError(
                f"Escalation policy '{engagement.policy_id}' was not found."
            )
        return policy
    ...
    def _notification_event_class_for(
        ...
        if purpose in {
            EngagementDeliveryPurpose.REMINDER,
            EngagementDeliveryPurpose.REPAGE,
        }:
    ...
    @staticmethod
    def _title_for_page(
        incident: IncidentRecord, purpose: EngagementDeliveryPurpose
    ) -> str:
        if purpose is EngagementDeliveryPurpose.HANDOFF:
