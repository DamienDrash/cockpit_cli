...
from cockpit.ops.events.health import ComponentWatchObserved
...
class ComponentWatchService:
    ...
    def probe_due_watches(
        self, *, now: datetime | None = None
    ) -> list[ComponentWatchState]:
    ...
    def probe_watch(
        self, watch_id: str, *, now: datetime | None = None
    ) -> ComponentWatchState:
        ...
        if config.subject_kind is WatchSubjectKind.DATASOURCE:
            ...
            raise ValueError(
                f"Unsupported watch subject kind '{config.subject_kind.value}'."
            )
    ...
    def _probe_datasource(
        ...
        try:
            ...
            return ComponentWatchState(
                ...
                last_outcome=WatchProbeOutcome.SUCCESS
                if success
                else WatchProbeOutcome.FAILURE,
    ...
    def _probe_docker_container(
        ...
        detail = next(
            (
                ...
                if item.container_id == config.subject_ref
                or item.name == config.subject_ref
            ),
            None,
        )
        if detail is None:
            ...
            healthy = (
                detail.health in {None, "", "healthy"}
            ) and detail.state == "running"
            success = healthy
            summary = (
