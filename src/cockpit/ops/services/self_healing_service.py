...
from cockpit.ops.services.recovery_policy_service import (
...
class SelfHealingService:
    ...
    def __init__(
        ...
        *,
        ...
        self._event_bus.subscribe(
            TaskExitedUnexpectedly, self._on_task_exited_unexpectedly
        )
        self._event_bus.subscribe(
            TunnelFailureDetected, self._on_tunnel_failure_detected
        )
        self._event_bus.subscribe(
            ComponentWatchObserved, self._on_component_watch_observed
        )
    ...
    def reset_quarantine(
        self, component_id: str, *, reason: str = "manual reset"
    ) -> None:
    ...
    def _run_pty_recovery(
        self, state: ComponentHealthState, attempt: RecoveryAttempt
    ) -> None:
        payload = self._runtime_payload(state)
        panel_id = str(
            payload.get("panel_id", "")
        ).strip() or state.component_id.removeprefix("pty:")
    ...
    def _run_tunnel_recovery(
        self, state: ComponentHealthState, attempt: RecoveryAttempt
    ) -> None:
        payload = self._runtime_payload(state)
        profile_id = str(
            payload.get("profile_id", "")
        ).strip() or state.component_id.removeprefix("ssh-tunnel:")
        try:
    ...
    def _run_task_recovery(
        ...
        ).strip() or state.component_id.removeprefix("task:")
        try:
    ...
    def _run_watch_recovery(
        self, state: ComponentHealthState, attempt: RecoveryAttempt
    ) -> None:
        probe_watch = getattr(
            self._component_watch_service, "probe_watch_for_component", None
        )
    ...
    def _on_pty_started(self, event: PTYStarted) -> None:
        ...
        state = self._component_health_repository.get(
            component_id
        ) or ComponentHealthState(
    ...
    def _on_task_heartbeat_missed(self, event: TaskHeartbeatMissed) -> None:
        component_id = str(
            event.metadata.get("component_id", f"task:{event.task_name}")
        )
        ...
        self._handle_failure(
            ...
            severity=IncidentSeverity.WARNING,
            target_kind=self._target_kind_from_metadata(
                event.metadata.get("target_kind")
            ),
            target_ref=(
    ...
    def _on_task_exited_unexpectedly(self, event: TaskExitedUnexpectedly) -> None:
        ...
        self._handle_failure(
            ...
            or f"task '{event.task_name}' exited unexpectedly",
            severity=IncidentSeverity.HIGH,
            target_kind=SessionTargetKind.LOCAL,
            target_ref=None,
            payload={
    ...
    def _handle_failure(
        ...
        *,
        ...
        pending_attempt = (
            self._recovery_attempt_repository.latest_pending_for_component(
        ...
        if evaluation.should_enter_cooldown:
            ...
            self._record_transition(state, previous_status, evaluation.explanation)
            self._set_incident_status(
                incident, IncidentStatus.OPEN, evaluation.explanation
            )
            self._incident_repository.add_timeline_entry(
                incident_id=incident.id,
    ...
    def _ensure_incident(
        ...
        *,
        ...
        incident = self._incident_repository.get_open_for_component(
            component_ref.component_id
        )
        now = self._now()
        if incident is None:
