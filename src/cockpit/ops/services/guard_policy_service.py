...
from cockpit.ops.models.policy import GuardContext, GuardDecision
...
class GuardPolicyService:
    ...
    def _evaluate_docker(self, context: GuardContext) -> GuardDecision:
        ...
        if context.action_kind is GuardActionKind.DOCKER_REMOVE:
            if (
                context.target_risk is TargetRiskLevel.PROD
                and not context.elevated_mode
            ):
    ...
    def _evaluate_shell(self, context: GuardContext) -> GuardDecision:
        ...
        if (
            context.action_kind is GuardActionKind.SHELL_DESTRUCTIVE
            and context.target_risk is TargetRiskLevel.PROD
        ):
            return self._decision(
                context,
        ...
        if (
            context.target_risk in {TargetRiskLevel.STAGE, TargetRiskLevel.PROD}
            and not context.elevated_mode
        ):
            return self._decision(
                context,
    ...
    def _evaluate_db(self, context: GuardContext) -> GuardDecision:
        ...
        if (
            context.action_kind is GuardActionKind.DB_DESTRUCTIVE
            and context.target_risk is TargetRiskLevel.PROD
        ):
            return self._decision(
                context,
    ...
    def _decision(
        ...
        *,
        ...
        return GuardDecision(
            ...
            requires_elevated_mode=outcome
            is GuardDecisionOutcome.REQUIRE_ELEVATED_MODE,
