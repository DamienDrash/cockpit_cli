...
class ChromaDatasourceAdapter:
    def inspect(self, profile: DataSourceProfile) -> DataSourceInspection:
        ...
        try:
            ...
            names = [getattr(collection, "name", str(collection)) for collection in collections[:20]]
            return DataSourceInspection(
                success=True,
                profile_id=profile.id,
                backend=profile.backend,
    ...
    def run(
        ...
        *,
        ...
        try:
            ...
            collection_name = str(payload.get("collection") or profile.database_name or "")
            ...
            if operation_name == "get":
                result = collection.get(ids=payload.get("ids"), where=payload.get("where"))
                return DataSourceOperationResult(
                    success=True,
                    profile_id=profile.id,
                    backend=profile.backend,
