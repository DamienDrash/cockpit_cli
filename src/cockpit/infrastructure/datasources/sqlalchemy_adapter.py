...
class SQLAlchemyDatasourceAdapter:
    ...
    def inspect(self, profile: DataSourceProfile) -> DataSourceInspection:
        ...
        try:
            ...
            with engine.connect() as connection:
                ...
                schema_names = []
                try:
                    schema_names = [str(name) for name in inspector.get_schema_names()[:12]]
                except Exception:
                    schema_names = []
                table_names = []
                try:
                    table_names = [str(name) for name in inspector.get_table_names()[:25]]
                except Exception:
                    table_names = []
            return DataSourceInspection(
                success=True,
            ...
            with engine.begin() as connection:
                ...
                if result.returns_rows:
                    ...
                    return DataSourceOperationResult(
                        ...
                        rows=[[self._stringify_cell(cell) for cell in row] for row in rows],
    ...
    def _engine(self, profile: DataSourceProfile):
        if not profile.connection_url:
            raise ValueError(f"Datasource '{profile.name}' is missing a connection URL.")
