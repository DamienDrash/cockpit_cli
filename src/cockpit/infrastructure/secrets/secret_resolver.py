...
class KeyringSecretProvider:
    ...
    def resolve(
        ...
        *,
        ...
        if value is None:
            raise ValueError(f"Keyring entry '{service}/{username}' could not be resolved.")
        return value
...
class SecretResolver:
    ...
    def _resolve_value(
        ...
        *,
        ...
        if isinstance(value, dict):
            return {
                str(key): self._resolve_value(item, refs, resolution_cache=resolution_cache)
                for key, item in value.items()
            }
        if isinstance(value, list):
            return [self._resolve_value(item, refs, resolution_cache=resolution_cache) for item in value]
        if isinstance(value, tuple):
            return tuple(self._resolve_value(item, refs, resolution_cache=resolution_cache) for item in value)
        return value
    ...
    def resolve_ref(
        ...
        *,
        ...
        provider_name = str(normalized.get("provider", "literal")).strip().lower() or "literal"
...
def _parse_vault_ref(ref: str) -> dict[str, object]:
    ...
    if not profile_id or "/" not in path:
        raise ValueError("Vault KV refs must look like vault://PROFILE/MOUNT/PATH#FIELD.")
