...
class VaultHttpClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._namespace = namespace.strip() if isinstance(namespace, str) and namespace.strip() else None
        self._verify_tls = verify_tls
        self._ca_cert_path = ca_cert_path.strip() if isinstance(ca_cert_path, str) and ca_cert_path.strip() else None
    ...
    def login_token(self, token: str) -> VaultAuthResult:
        ...
        return VaultAuthResult(
            token=token,
            token_accessor=str(data.get("accessor")) if data.get("accessor") is not None else None,
    ...
    def kv_read(
        ...
        *,
        ...
        query = {"version": str(version)} if isinstance(version, int) and version > 0 else None
        payload = self._request_json(
            "GET",
    ...
    def _ssl_context(self) -> ssl.SSLContext | None:
        ...
        if self._ca_cert_path:
            return ssl.create_default_context(cafile=str(Path(self._ca_cert_path).expanduser()))
...
def _auth_result_from_payload(
    ...
    *,
    ...
    return VaultAuthResult(
        token=token,
        token_accessor=(
            str(auth.get("accessor"))
            if auth.get("accessor") is not None
            else None
        ),
