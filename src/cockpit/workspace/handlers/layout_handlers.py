...
class ToggleActiveLayoutOrientationHandler:
    ...
    def __call__(self, command: Command) -> DispatchResult:
        ...
        active_split["orientation"] = (
            "horizontal" if current == "vertical" else "vertical"
        )
        return DispatchResult(
            success=True,
...
class FocusNextPanelHandler:
    ...
    def __call__(self, command: Command) -> DispatchResult:
        ...
        ordered_ids = [
            panel_id for panel_id in visible_panel_ids if isinstance(panel_id, str)
        ]
        if not ordered_ids:
            raise CommandContextError("No visible panels are available.")
...
def _active_root_split(
    tabs: list[dict[str, object]], active_tab: str
) -> dict[str, object]:
    for tab in tabs:
