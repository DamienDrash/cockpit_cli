...
from cockpit.core.config import config_dir, layouts_dir, themes_dir
...
class ConfigLoader:
    ...
    def load_keybindings(self) -> dict[str, object]:
        return self._load_yaml(
            config_dir(self._start) / "keybindings.yaml", required=False
        )

    def load_command_catalog(self) -> dict[str, object]:
        return self._load_yaml(
            config_dir(self._start) / "commands.yaml", required=False
        )

    def load_connections(self) -> dict[str, object]:
        return self._load_yaml(
            config_dir(self._start) / "connections.yaml", required=False
        )
    ...
    def load_datasources(self) -> dict[str, object]:
        return self._load_yaml(
            config_dir(self._start) / "datasources.yaml", required=False
        )
