...
class LayoutService:
    ...
    def add_panel_to_tab(
        ...
        *,
        ...
        for tab in layout.tabs:
            ...
            if len(existing_children) == 1 and isinstance(
                existing_children[0], PanelRef
            ):
        ...
        *,
        ...
        for index, child in enumerate(node.children):
            ...
            if isinstance(child, PanelRef):
                ...
                if (
                    allowed_panel_types is not None
                    and child.panel_type not in allowed_panel_types
                ):
                    errors.append(
                        f"{child_path} panel_type '{child.panel_type}' is not registered."
                    )
                if (
                    allowed_panel_ids is not None
                    and child.panel_id not in allowed_panel_ids
                ):
                    errors.append(
                        f"{child_path} panel_id '{child.panel_id}' is not registered."
                    )
                continue
            errors.extend(
                self._validate_split_node(
    ...
    def _decode_split_node(self, raw_node: object) -> SplitNode:
        ...
        for child in raw_children if isinstance(raw_children, list) else []:
            if isinstance(child, dict) and {"panel_id", "panel_type"} <= set(
                child.keys()
            ):
                children.append(
                    PanelRef(
        ...
        return SplitNode(
            ...
            ratio=float(raw_node["ratio"])
            if raw_node.get("ratio") is not None
            else None,
            children=children,
        )
    ...
    def _clone_tab(self, tab: TabLayout) -> TabLayout:
        return TabLayout(
            id=tab.id, name=tab.name, root_split=self._clone_split(tab.root_split)
        )
    ...
    def _clone_split(self, node: SplitNode) -> SplitNode:
        ...
        return SplitNode(
            orientation=node.orientation, ratio=node.ratio, children=children
        )
    ...
    def _remove_panel(self, node: SplitNode, panel_id: str) -> SplitNode | PanelRef:
        ...
        if len(next_children) == 1 and isinstance(next_children[0], SplitNode):
            return next_children[0]
        return SplitNode(
            orientation=node.orientation, ratio=node.ratio, children=next_children
        )
    ...
    def _replace_panel(
        ...
        for child in node.children:
            if isinstance(child, PanelRef):
                ...
                next_children.append(
                    self._replace_panel(child, existing_panel_id, replacement)
                )
        return SplitNode(
            orientation=node.orientation, ratio=node.ratio, children=next_children
        )
    ...
    def _move_panel(
        ...
        next_children[index], next_children[target] = (
