...
class DBPanel(BasePanel):
                ...
                with ContentSwitcher(initial="ide", id="db-main-switcher"):
                    ...
                    with Vertical(id="config"):
                        ...
                        with Vertical(id="db-config-form"):
                            ...
                            yield Label("Backend:")
                            yield Select([("SQLite", "sqlite"), ("PostgreSQL", "postgres")], value="sqlite", id="db-cfg-backend")
                            yield Label("URL:")
                            yield Input(placeholder="sqlite:///db.sqlite", id="db-cfg-url")
    ...
    def _update_catalog_tree(self, snapshot: DatabaseCatalogSnapshot, profiles: list[object]) -> None:
        ...
        for profile in profiles:
            ...
            node = tree.root.add(f"  {name}", data={"kind": "profile", "id": pid}, expand=False)
            # Add a dummy node to allow expansion
            node.add("loading...", data={"kind": "loading"})
            
        # 2. Local SQLite Files
        for path in snapshot.databases:
            node = tree.root.add(f"  {Path(path).name}", data={"kind": "sqlite_file", "path": path}, expand=False)
            node.add("loading...", data={"kind": "loading"})
    ...
    def _load_profile_tables(self, node: Tree.Node, profile_id: str) -> None:
        ...
        try:
            ...
            if inspect.success:
                ...
                for t in tables:
                    node.add_leaf(f"  {t}", data={"kind": "table", "name": t, "pid": profile_id})
                if not tables: node.add_leaf("(no tables)", data={})
    ...
    def _load_sqlite_tables(self, node: Tree.Node, path: str) -> None:
        ...
        try:
            ...
            for t in tables:
                node.add_leaf(f"  {t}", data={"kind": "sqlite_table", "name": t, "path": path})
            if not tables: node.add_leaf("(no tables)", data={})
        except Exception as exc:
            node.add_leaf(f"Error: {exc}", data={})
