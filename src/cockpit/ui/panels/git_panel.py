...
from cockpit.infrastructure.git.git_adapter import GitAdapter, GitFileStatus
...
class GitPanel(BasePanel):
    ...
    def compose(self) -> ComposeResult:
        with Horizontal():
            ...
            with Vertical(id="git-main"):
                ...
                yield Static(
                    "Select a file to see diff", id="git-diff-view", classes="diff-view"
                )
    ...
    def _update_diff(self) -> None:
        ...
        try:
            ...
            self.query_one("#git-diff-view", Static).update(
                Syntax(diff_text, "diff", theme="monokai")
            )
        except Exception as exc:
            self.query_one("#git-diff-view", Static).update(
                f"[red]Diff failed: {exc}[/red]"
            )
    ...
    def _move_selection(self, delta: int) -> None:
        ...
        self._selected_index = max(
            0, min(len(self._files) - 1, self._selected_index + delta)
        )
        self._render_all()
    ...
    def _stage_selected(self) -> None:
        ...
        if self._git_adapter.stage_file(
            self._repo_root,
            f.path,
            target_kind=self._target_kind,
            target_ref=self._target_ref,
    ...
    def _unstage_selected(self) -> None:
        if not self._files:
            return
        f = self._files[self._selected_index]
        if self._git_adapter.unstage_file(
            self._repo_root,
            f.path,
            target_kind=self._target_kind,
            target_ref=self._target_ref,
