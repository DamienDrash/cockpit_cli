...
class WorkPanel(BasePanel):
    ...
    def compose(self) -> ComposeResult:
        with Horizontal():
            ...
            with Vertical(id="work-main"):
                yield Static(
                    "Terminal initializing...", id="work-note", classes="pane-title"
                )
                yield EmbeddedTerminal(
                    on_input=lambda data: self._pty_manager.send_input(
                        self.PANEL_ID, data
                    )
                )
    ...
    def on_key(self, event: events.Key) -> None:
        ...
        if ansi:
            try:
                self._pty_manager.send_input(self.PANEL_ID, ansi)
            except Exception:
                pass
            event.stop()
    ...
    def _sync_explorer(self) -> None:
        try:
            explorer = self.query_one(FileExplorer)
            explorer.load(
                root_path=self._workspace_root, browser_path=self._workspace_root
            )
        except NoMatches:
            pass
    ...
    def _on_runtime_event(self, event: object) -> None:
        if getattr(event, "panel_id", None) != self.PANEL_ID:
            return
