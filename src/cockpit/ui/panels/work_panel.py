...
class WorkPanel(BasePanel):
    ...
    def attach_terminal(self) -> None:
        ...
        try:
            self._pty_manager.start_session(self.PANEL_ID, self._cwd, target_kind=self._target_kind, target_ref=self._target_ref)
    ...
    def _map_key_to_ansi(self, event: events.Key) -> str | None:
        if event.key.startswith("ctrl+"):
            ...
            if len(part) == 1 and "A" <= part <= "Z": return chr(ord(part) - 64)
            return None
        mapping = {"enter": "\r", "tab": "\t", "backspace": "\x7f", "up": "\x1b[A", "down": "\x1b[B", "left": "\x1b[D", "right": "\x1b[C", "escape": "\x1b"}
    ...
    def _handle_explorer_key(self, event: events.Key) -> None:
        ...
        if event.key == "up": explorer.move_selection(-1)
            ...
            if Path(self._selected_path).is_file():
                # Open in terminal with nano
                cmd = f"\x03\x0c nano {shlex.quote(self._selected_path)}\r"
    ...
    def _render_context(self) -> None:
        try:
            ...
            self.query_one(FileContext).update_context(workspace_name=self._workspace_name, workspace_root=self._workspace_root, cwd=self._cwd, selected_path=self._selected_path, restored=self._restored, target_label=self._target_kind.value, risk_label=risk_presentation(level).label)
        except NoMatches: pass

    def _on_runtime_event(self, event: object) -> None:
        if getattr(event, "panel_id", None) != self.PANEL_ID: return
        if get_ident() != self._main_thread_id: self.app.call_from_thread(self._apply_runtime_event, event)
        else: self._apply_runtime_event(event)
    ...
    def _apply_runtime_event(self, event: object) -> None:
        try:
            ...
            elif isinstance(event, ProcessOutputReceived): terminal.append_output(event.chunk)
        except NoMatches: pass
    ...
    def command_context(self) -> dict[str, object]:
        return {"panel_id": self.PANEL_ID, "workspace_root": self._workspace_root, "cwd": self._cwd}
