...
from cockpit.shared.enums import ApprovalDecisionKind
...
class StartResponseRunHandler:
    ...
    def __call__(self, command: Command) -> DispatchResult:
        argv = _argv(command)
        if len(argv) < 2:
            raise CommandContextError("Response start requires an incident id and runbook id.")
...
class ExecuteResponseStepHandler:
    ...
    def __call__(self, command: Command) -> DispatchResult:
        ...
        return DispatchResult(
            success=True,
            message=response_run.summary or f"Executed response step for {response_run.id}.",
            data={"response_run_id": response_run.id},
        )
...
class RetryResponseStepHandler:
    ...
    def __call__(self, command: Command) -> DispatchResult:
        ...
        return DispatchResult(
            success=True,
            message=response_run.summary or f"Retried response step for {response_run.id}.",
            data={"response_run_id": response_run.id},
        )
...
class CompensateResponseRunHandler:
    ...
    def __call__(self, command: Command) -> DispatchResult:
        ...
        return DispatchResult(
            success=True,
            message=response_run.summary or f"Compensation handled for {response_run.id}.",
            data={"response_run_id": response_run.id},
        )
...
class DecideApprovalHandler:
    ...
    def __call__(self, command: Command) -> DispatchResult:
        ...
        return DispatchResult(
            ...
            data={"response_run_id": response_run.id, "approval_request_id": request_id},
        )
