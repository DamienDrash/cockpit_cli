...
class OnCallResolutionService:
    ...
    def resolve_ownership(
        ...
        *,
        ...
        escalation_policy_id = binding.escalation_policy_id or team.default_escalation_policy_id
    ...
    def resolve_oncall(self, *, team_id: str, effective_at: datetime) -> OnCallResolution:
        ...
        schedules = [
            schedule
            for schedule in self._schedule_repository.list_for_team(team.id, enabled_only=True)
        ...
        rotations = self._rotation_repository.list_for_schedule(schedule.id, enabled_only=True)
        if not rotations:
            return OnCallResolution(
                outcome=ResolutionOutcome.BLOCKED,
                team_id=team.id,
        ...
        active_overrides = [
            ...
            if override.replaced_person_id is None or override.replaced_person_id == base_person_id
        ]
        if active_overrides:
    ...
    def resolve_recipient(
        ...
        *,
        ...
        team_resolution = self.resolve_oncall(team_id=target_ref, effective_at=effective_at)
        if team_resolution.outcome is not ResolutionOutcome.RESOLVED or not team_resolution.person_id:
            return ResolvedEscalationRecipient(
    ...
    def _binding_matches(
        *,
        ...
        if binding_component_kind is not None and binding_component_kind is not component_kind:
            return False
        if binding_component_id is not None and binding_component_id != component_id:
            return False
        if binding_subject_kind is not None and binding_subject_kind is not subject_kind:
            return False
    ...
    def _schedule_matches(self, schedule: OnCallSchedule, effective_at: datetime) -> bool:
        ...
        return (
            (weekday in days and minute_of_day >= start_minutes)
            or (previous_weekday in days and minute_of_day < end_minutes)
        )

    def _resolve_rotation_person(self, rotation: RotationRule, effective_at: datetime) -> str:
        ...
        interval_seconds = self._interval_seconds(rotation.interval_kind, rotation.interval_count)
        step_index = int(elapsed_seconds // interval_seconds) if interval_seconds > 0 else 0
    ...
    def _channel_ids_for_person(person: OperatorPerson) -> tuple[str, ...]:
        targets = sorted(
            (target for target in person.contact_targets if target.enabled and target.channel_id),
