...
class ActivityLogService:
    ...
    @staticmethod
    def _record_from_command_payload(payload: dict[str, object]) -> ActivityLogRecord | None:
        command_id = payload.get("command_id")
        name = payload.get("name")
        recorded_at = payload.get("recorded_at")
        if not isinstance(command_id, str) or not isinstance(name, str) or not isinstance(
            recorded_at, str
        ):
            return None
    ...
    @staticmethod
    def _record_from_audit_payload(payload: dict[str, object]) -> ActivityLogRecord | None:
        command_id = payload.get("command_id")
        action = payload.get("action")
        recorded_at = payload.get("recorded_at")
        if not isinstance(command_id, str) or not isinstance(action, str) or not isinstance(
            recorded_at, str
        ):
            return None
