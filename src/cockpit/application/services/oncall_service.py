...
from cockpit.shared.enums import ScheduleCoverageKind
...
class OnCallService:
    ...
    def save_person(self, person: OperatorPerson) -> OperatorPerson:
        ...
        for existing in self._person_repository.list_all():
            if existing.id != person.id and existing.handle.lower() == person.handle.lower():
    ...
    def save_team(self, team: OperatorTeam) -> OperatorTeam:
        ...
        if team.default_escalation_policy_id:
            policy = self._escalation_policy_repository.get(team.default_escalation_policy_id)
            if policy is None:
                raise OnCallValidationError(
    ...
    def list_memberships_for_team(
        ...
        *,
        ...
        return self._membership_repository.list_for_team(team_id, enabled_only=enabled_only)
    ...
    def save_membership(self, membership: TeamMembership) -> TeamMembership:
        ...
        if self._person_repository.get(membership.person_id) is None:
            raise OnCallValidationError(f"Operator '{membership.person_id}' was not found.")
        for existing in self._membership_repository.list_for_team(membership.team_id):
            if existing.id != membership.id and existing.person_id == membership.person_id:
                raise OnCallValidationError("Duplicate team membership is not allowed.")
        now = utc_now()
        existing = next(
            (item for item in self._membership_repository.list_all() if item.id == membership.id),
            None,
        )
        persisted = replace(
            membership,
    ...
    def list_ownership_bindings(self, *, enabled_only: bool = False) -> list[OwnershipBinding]:
    ...
    def save_ownership_binding(self, binding: OwnershipBinding) -> OwnershipBinding:
        ...
        if binding.escalation_policy_id:
            policy = self._escalation_policy_repository.get(binding.escalation_policy_id)
            if policy is None:
                raise OnCallValidationError(
    ...
    def list_schedules_for_team(
        ...
        *,
        ...
        return self._schedule_repository.list_for_team(team_id, enabled_only=enabled_only)
    ...
    def save_rotation(self, rotation: RotationRule) -> RotationRule:
        ...
        if schedule is None:
            raise OnCallValidationError(f"Schedule '{rotation.schedule_id}' was not found.")
        ...
        memberships = {
            item.person_id
            for item in self._membership_repository.list_for_team(schedule.team_id, enabled_only=True)
        }
        missing = [person_id for person_id in rotation.participant_ids if person_id not in memberships]
        if missing:
            raise OnCallValidationError(
        ...
        enabled_rotations = [
            item
            for item in self._rotation_repository.list_for_schedule(schedule.id, enabled_only=True)
    ...
    def save_override(self, override: ScheduleOverride) -> ScheduleOverride:
        ...
        if schedule is None:
            raise OnCallValidationError(f"Schedule '{override.schedule_id}' was not found.")
        ...
        if override.replaced_person_id and self._person_repository.get(override.replaced_person_id) is None:
            raise OnCallValidationError(
        ...
        for existing in self._override_repository.list_for_schedule(schedule.id, enabled_only=True):
    ...
    def _validate_schedule(self, schedule: OnCallSchedule) -> None:
        ...
        if not schedule.enabled:
            return
        for existing in self._schedule_repository.list_for_team(schedule.team_id, enabled_only=True):
    ...
    def _schedule_windows(self, schedule: OnCallSchedule) -> list[tuple[int, int]]:
        ...
        if not isinstance(raw_days, list) or not raw_days:
            raise OnCallValidationError("Weekly schedules require a non-empty 'days' list.")
        days = []
        for item in raw_days:
            if not isinstance(item, int) or item < 0 or item > 6:
                raise OnCallValidationError("Schedule days must be integers from 0 to 6.")
            days.append(item)
        ...
        if start_minutes == end_minutes:
            raise OnCallValidationError("Schedule start and end time cannot be identical.")
