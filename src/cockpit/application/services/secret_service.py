...
from cockpit.domain.models.secret import (
    ManagedSecretEntry,
    VaultLease,
    VaultProfile,
    VaultSession,
...
from cockpit.infrastructure.secrets.vault_client import (
    VaultAuthResult,
    VaultHttpClient,
    VaultLeaseResult,
)
...
class SecretService:
    ...
    def rotate_entry(self, name: str, *, secret_value: str) -> ManagedSecretEntry:
        ...
        if entry.provider == "keyring":
            ...
            if not profile_id or not mount or not path or not field:
                raise ValueError("Vault KV secret is missing its profile, mount, path, or field.")
            profile = self.require_vault_profile(profile_id)
            session = self._ensure_vault_session(profile.id)
            ...
            raise ValueError("Only keyring-backed or Vault KV secrets support direct rotation.")
    ...
    def save_vault_profile(
        ...
        *,
        ...
        profile = VaultProfile(
            ...
            auth_mount=auth_mount.strip() if isinstance(auth_mount, str) and auth_mount.strip() else None,
            role_name=role_name.strip() if isinstance(role_name, str) and role_name.strip() else None,
            namespace=namespace.strip() if isinstance(namespace, str) and namespace.strip() else None,
            description=description.strip() if isinstance(description, str) and description.strip() else None,
            verify_tls=bool(verify_tls),
            ca_cert_path=ca_cert_path.strip() if isinstance(ca_cert_path, str) and ca_cert_path.strip() else None,
            allow_local_cache=bool(allow_local_cache),
            cache_ttl_seconds=max(int(cache_ttl_seconds or 0), 0) or 3600,
            risk_level=risk_level.strip() if isinstance(risk_level, str) and risk_level.strip() else "dev",
            tags=[item.strip() for item in (tags or []) if isinstance(item, str) and item.strip()],
            created_at=existing.created_at if existing is not None else timestamp,
            updated_at=timestamp,
        )
        ...
        *,
        ...
        if auth_type == "token":
            ...
            if not role or not raw_jwt:
                raise ValueError("Vault JWT/OIDC auth requires a configured role and a JWT token.")
    ...
    def vault_profile_health(self, profile_id: str) -> dict[str, object]:
        ...
        session = self._vault_sessions.get(profile_id) or self._load_cached_session(profile)
        return {
        ...
        *,
        ...
        if kind == "kv":
            ...
            if not isinstance(data, dict) or field not in data:
                raise ValueError(f"Vault KV field '{field}' was not found at {mount}/{path}.")
            value = data[field]
            return str(value)
        if kind == "dynamic":
            mount = str(reference.get("mount", "")).strip()
            ...
            if not mount or not role or not field:
                raise ValueError("Vault dynamic references require mount, role, and field.")
            ...
            if lease_result is None:
                ...
                self._record_lease(profile.id, kind="dynamic", mount=mount, path=f"creds/{role}", result=lease_result)
                if resolution_cache is not None:
    ...
    def transit_operation(
        ...
        *,
        ...
        if normalized_operation == "decrypt":
            ...
            decoded = b64decode(plaintext_b64.encode("utf-8")).decode("utf-8") if plaintext_b64 else ""
    ...
    def diagnostics(self) -> SecretDiagnostics:
        ...
        return SecretDiagnostics(
            ...
            active_vault_sessions=sum(1 for session in sessions if session.source == "live"),
    ...
    def _reference_from_inputs(
        ...
        *,
        ...
        if provider == "vault":
            ...
            raise ValueError("Managed Vault secrets only support kv or dynamic references.")
        raise ValueError("Managed secrets only support env, file, keyring, or vault providers.")
    ...
    def _session_from_auth_result(self, profile: VaultProfile, auth: VaultAuthResult) -> VaultSession:
        return VaultSession(
            profile_id=profile.id,
            auth_type=profile.auth_type,
    ...
    def _require_token(self, profile_id: str) -> str:
        token = self._vault_tokens.get(profile_id)
        if not token:
            raise RuntimeError(f"Vault session token for '{profile_id}' is not available.")
        return token
    ...
    def _profile_from_payload(payload: dict[str, object] | None) -> VaultProfile | None:
        ...
        if not all(isinstance(item, str) and item for item in (profile_id, name, address, auth_type)):
            return None
        return VaultProfile(
            id=profile_id,
            name=name,
            address=address,
            auth_type=auth_type,
            auth_mount=str(payload["auth_mount"]) if payload.get("auth_mount") is not None else None,
            role_name=str(payload["role_name"]) if payload.get("role_name") is not None else None,
            ...
            ca_cert_path=str(payload["ca_cert_path"]) if payload.get("ca_cert_path") is not None else None,
            allow_local_cache=bool(payload.get("allow_local_cache", False)),
            cache_ttl_seconds=max(int(payload.get("cache_ttl_seconds", 3600) or 3600), 1),
    ...
    def _lease_from_payload(payload: dict[str, object] | None) -> VaultLease | None:
        ...
        if not all(isinstance(item, str) and item for item in (lease_id, profile_id, source_kind, mount, path)):
            return None
