...
from cockpit.domain.models.health import ComponentHealthState, RecoveryAttempt, RecoveryPolicy
from cockpit.shared.enums import ComponentKind, HealthStatus
...
class RecoveryPolicyService:
    ...
    def policy_for(self, component_kind: ComponentKind) -> RecoveryPolicy:
        return self._policies.get(component_kind, self._default_policies()[component_kind])
        ...
        *,
        ...
        if any(marker in lower_reason for marker in policy.non_recoverable_markers):
            return RecoveryEvaluation(
                ...
                attempt_number=max((attempt.attempt_number for attempt in recent_attempts), default=0),
                backoff_seconds=0,
                cooldown_until=None,
        ...
        if state is not None and state.cooldown_until is not None and state.cooldown_until > effective_now:
            return RecoveryEvaluation(
                should_schedule_attempt=False,
                should_enter_cooldown=False,
        ...
        window_start = effective_now - timedelta(seconds=max(1, policy.retry_window_seconds))
        in_window = [
            attempt
            for attempt in recent_attempts
            if (
                (attempt.finished_at or attempt.started_at or attempt.scheduled_for)
                and (attempt.finished_at or attempt.started_at or attempt.scheduled_for) >= window_start
            )
        ]
        attempts_used = len(in_window)
        next_attempt_number = max((attempt.attempt_number for attempt in recent_attempts), default=0) + 1
        ...
        cooldown_until = effective_now + timedelta(seconds=max(1, policy.cooldown_seconds))
    ...
    def _default_policies() -> dict[ComponentKind, RecoveryPolicy]:
        return {
            ComponentKind.PTY_SESSION: RecoveryPolicy(
                ...
                cooldown_seconds=60,
                non_recoverable_markers=("permission denied", "no such file", "not found"),
            ),
            ComponentKind.SSH_TUNNEL: RecoveryPolicy(
                ...
                non_recoverable_markers=("permission denied", "host key verification failed"),
            ),
            ...
            ComponentKind.PLUGIN_HOST: RecoveryPolicy(
                ...
                cooldown_seconds=120,
                non_recoverable_markers=("permission denied", "integrity_failed", "incompatible"),
            ),
