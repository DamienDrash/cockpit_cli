...
class NotificationService:
    ...
    def __init__(
        ...
        *,
        ...
        self._event_bus.subscribe(ComponentHealthChanged, self._on_component_health_changed)
    ...
    def list_failed_deliveries(self, *, limit: int = 25) -> list[NotificationDeliveryAttempt]:
    ...
    def notification_detail(self, notification_id: str) -> dict[str, object] | None:
        ...
        return {
            ...
            "deliveries": [
                attempt.to_dict()
                for attempt in self._delivery_repository.list_for_notification(notification_id)
            ],
        }
    ...
    def summary(self) -> dict[str, object]:
        return {
            ...
            "recent_failures": [item.to_dict() for item in self.list_failed_deliveries(limit=10)],
        }
    ...
    def _enqueue(self, candidate: NotificationCandidate) -> None:
        ...
        notification = NotificationRecord(
            ...
            status=NotificationStatus.SUPPRESSED if suppressed else NotificationStatus.QUEUED,
    ...
    def _deliver_attempt(self, attempt: NotificationDeliveryAttempt) -> None:
        ...
        channels = {
            channel.id: channel
            for channel in self._notification_policy_service.list_channels(enabled_only=True)
        }
        ...
        if channel.kind is NotificationChannelKind.INTERNAL:
            ...
            attempt.finished_at = utc_now()
            attempt.response_payload = {"message": "internal notification available in Cockpit views"}
            self._delivery_repository.save(attempt)
            self._event_bus.publish(
                NotificationDelivered(
                    delivery_id=attempt.id,
        ...
        try:
            resolved_target = self._secret_resolver.resolve_value(channel.target, channel.secret_refs)
            if not isinstance(resolved_target, dict):
                raise NotificationAdapterError("Resolved notification target must be a mapping.")
    ...
    def _schedule_retry_if_needed(
        ...
        backoff_seconds = min(
            ...
            int(channel.base_backoff_seconds) * (2 ** max(0, attempt.attempt_number - 1)),
        )
    ...
    def _sync_notification_status(self, notification_id: str) -> None:
        ...
        elif statuses <= {NotificationDeliveryStatus.SUCCEEDED, NotificationDeliveryStatus.SUPPRESSED}:
            notification.status = NotificationStatus.DELIVERED
        elif NotificationDeliveryStatus.RUNNING in statuses or NotificationDeliveryStatus.SCHEDULED in statuses:
    ...
    def _payload_for(self, notification: NotificationRecord, channel) -> NotificationDeliveryPayload:
        ...
        return NotificationDeliveryPayload(
            ...
            metadata={
                ...
                "component_kind": notification.component_kind.value if notification.component_kind else None,
            },
        )
    ...
    def _adapter_for(self, kind: NotificationChannelKind) -> NotificationAdapter:
        ...
        if adapter is None:
            raise NotificationAdapterError(f"No adapter is registered for channel kind '{kind.value}'.")
        return adapter
