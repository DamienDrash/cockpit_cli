    ...
    def inspect(self, profile: DataSourceProfile) -> DataSourceInspection:
        ...
        try:
            ...
            if profile.database_name:
                details["collections"] = [
                    str(item)
                    for item in client[profile.database_name].list_collection_names()[
                        :20
                    ]
                ]
            return DataSourceInspection(
        ...
        *,
        ...
        try:
            ...
            if not database_name or not collection_name:
                raise ValueError(
                    "MongoDB payload must include database and collection."
                )
            collection = client[database_name][collection_name]
            if op_name == "find":
                cursor = collection.find(
                    payload.get("filter", {}), payload.get("projection")
                )
                rows = [
                    json.dumps(document, default=str, sort_keys=True)
                    for document in cursor.limit(max(1, int(row_limit)))
                ]
                return DataSourceOperationResult(
                    success=True,
                    profile_id=profile.id,
