...
class RunDatabaseQueryHandler:
    ...
    def __call__(self, command: Command) -> DispatchResult:
        ...
        self._enforce_guard_decision(
            command, decision, display_name=Path(database_path).name or database_path
        )
        ...
        *,
        ...
        if profile is None:
            raise CommandContextError(
                f"Datasource profile '{profile_id}' was not found."
            )
    ...
    def _resolve_query(self, command: Command) -> str:
        ...
        if len(query_tokens) >= 2 and query_tokens[0].endswith(
            (".db", ".sqlite", ".sqlite3")
        ):
        ...
        *,
        ...
        return self._guard_policy_service.evaluate(
            GuardContext(
                ...
                workspace_name=self._optional_str(
                    command.context.get("workspace_name")
                ),
                target_ref=self._optional_str(command.context.get("target_ref")),
