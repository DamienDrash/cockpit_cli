...
from cockpit.datasources.adapters.backends.base import DataSourceAdapter
from cockpit.datasources.adapters.backends.chroma_adapter import ChromaDatasourceAdapter
...
from cockpit.datasources.adapters.backends.url_tools import (
...
class DataSourceService:
    ...
    def create_profile(
        ...
        *,
        ...
        profile = DataSourceProfile(
            ...
            if isinstance(connection_url, str) and connection_url.strip()
            else None,
            ...
            capabilities=list(
                DEFAULT_CAPABILITIES.get(normalized_backend, ["can_query"])
            ),
    ...
    def inspect_profile(self, profile_id: str) -> DataSourceOperationResult:
        ...
        return (
            self._adapter_for(prepared_profile)
            .inspect(prepared_profile)
            .to_operation_result()
        )
            ...
            capabilities=[
                ...
                for item in payload.get(
                    ...
                    DEFAULT_CAPABILITIES.get(
                        str(payload.get("backend", "sqlite")).lower(), ["can_query"]
                    ),
                )
